...
This discussion is going to be fairly light and in the realm of the hypothetical,
and therefore not include any code changes to our experimental recompiler.

We're going to explore how we might manipulate the instructions before recompiling,
to make the recompiler's job easier.

This is going to include a brief summary of a technique called **Static single-assignment**
and how it may be useful in our use case.

We'll then spend a bit of time examining when it might be a bad idea to optimise too much
and, given the context, talk a bit about two of the rather more interesting MIPS instructions,
**LWL** and **LWR**.
...
Maybe it's time to bring a degree of realism to the imaginary register **R32**.
...
and hopefully we too might be able to see some benefits.
...
In the [next][part-next] and final part, we'll deal with the last class of instruction, branches.
