...
the **load delay-slot**. First we'll outline how an in an interpreter might go about handling 
such subtleties before moving on to a basic implementation for our recompiler.
...
We'll also introduce state to our recompiler. This will allow us to keep track of when we're 
dealing with load delay-slot.
...
outside world similar **StoreWord** discussed in the [previous part]({% post_url 2020-04-07-part-06-0 %}).
...
Turning our attention to the recompiler, the **EmitLw** function we're hoping to achieve 
...
during execution and the value we used during recompilation will be wrong.
...
The recompiler loop will then look something like
...
On the left-hand side we have the state of **RSP** and **RBP** after pushing the caller's 
**RBP** and  on the right-hand side we have the state of **RSP** and **RBP** after subtracting 
16 from **RSP**.
...
The implementation of **CallLoadWord** (recalling **CallStoreWord** from the previous part) is as
follows,
...
Our implementation of **EmitLw** now looks like
...
In the next section we'll discuss how to complete delayed loads.

## The Persistence of Memory

In order to complete a delayed load we're going to have to move the load delayed value
from the stack back into the **R3051** register array.

Given a Guest register **rt** we want to write to we can achieve this in three steps
...
and the **EmitLW** method is nearly complete with this addition
...
and similarly for the interpreter loop
...
In the penultimate discussion we'll discuss exception handling.
...
delay-state can be written back using the **CallInterpreterFunction** developed in
[Part 3][part-03-0].

The load delayed value requires a modification of the **CallInterpreterFunction** and
given a function
...
enables the call to SetLoadDelayValue by loading the contents of **RBP - 4** into **RSI** as per the System V ABI
requirements for passing the second argument.
...
Note here that in the event of an exception we can no longer be in a load-delay slot,
(because we just emitted code to complete the last delayed load) so the state is written 
back as such.

The recompiled epilogue takes the form
...
which takes the values from the recompiler state and writes them back to the guest processor.
